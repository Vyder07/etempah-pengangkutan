...
<body class="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">
        ...
        <div class="flex-1 max-w-xl">
            <div class="relative">
                <input
                    id="globalSearch"
                    type="search"
                    placeholder="@yield('search-placeholder', 'Cari tempahan, pengguna, kenderaan...')"
    ...
    <script>
        ...
        // Global search functionality across modules
        ...
        function displaySearchResults(data, query) {
            if (!data.bookings?.length && !data.users?.length) {
                searchResultsContent.innerHTML = `
            ...
            if (data.bookings && data.bookings.length > 0) {
                html += `
                    <div class="px-3 py-2 bg-gray-50 border-b">
                        <span class="text-xs font-semibold text-gray-600 uppercase">Tempahan (${data.bookings.length})</span>
                    </div>
                `;
                data.bookings.forEach(booking => {
                    ...
                    html += `
                        <a href="/admin/booking?highlight=${booking.id}" class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-100 transition-colors">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-sm text-gray-600 mt-0.5">${userName} • ${destination}</p>
            ...
            if (data.users && data.users.length > 0) {
                html += `
                    <div class="px-3 py-2 bg-gray-50 border-b">
                        <span class="text-xs font-semibold text-gray-600 uppercase">Pengguna (${data.users.length})</span>
                    </div>
                `;
                data.users.forEach(user => {
                    ...
                    html += `
                        <div class="px-4 py-3 hover:bg-gray-50 border-b border-gray-100 transition-colors cursor-pointer">
                            <div class="flex items-center gap-3">
                                <span class="material-icons text-gray-600">person</span>
                                <div class="flex-1">
                                    <div class="flex items-center gap-2">
                                        <span class="font-medium text-gray-900">${user.name}</span>
                                        <span class="text-xs px-2 py-0.5 rounded ${roleClass}">${user.role}</span>
                                    </div>
